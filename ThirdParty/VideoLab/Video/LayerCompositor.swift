...
class LayerCompositor {
    ...
    private func blendOutputText(_ outputTexture: Texture,
        ...
        // Render
        var backgroundClone: Texture?
        if blendOperation.requiresBackgroundTexture {
            guard let clone = cloneTexture(from: outputTexture) else {
                return
            }
            backgroundClone = clone
            blendOperation.enableOutputTextureRead = false
            blendOperation.addTexture(clone, at: 1)
        } else {
